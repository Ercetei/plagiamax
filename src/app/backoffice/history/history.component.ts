...
export class HistoryComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.generalService.get("/user/" + this.currentIdUser + "/bets").then( data => {
      ...
      parent.allBetUser.forEach(element => {
        ...
        if(element.betlines.length > 1) {
          ...
          element.betlines.forEach(bl => {
            this.generalService.get("/bettype/" + bl.bettype.id + "/teams").then( dataTeams => {
              let teams:string = "";
        ...
        // if simple bet
          ...
          this.generalService.get("/bettype/" + element.betlines[0].bettype.id + "/teams").then( dataTeams => {
            let teams:string;
